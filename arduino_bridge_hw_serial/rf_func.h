...
void readMessage() {
  ...
  if (millis() - read_rf_timer > READ_RF_DELAY) {  // if card was in memory  and  card hold delay has passed -
    read_rf_timer = millis();  // update timer for holding card in memory
    return;
  }
